...
GENERATED_ROOT_CONF_FILE="/work/ca.conf"
...
openssl req -new -x509 -days 365 -key /work/ca.key -out /work/ca.pem -config /work/ca.conf

echo "Printing root CA certificate"
openssl x509 -in /work/ca.pem -noout -text

echo "---> Please share ca.pem with Temporal cloud"
